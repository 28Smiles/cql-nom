...
use crate::utils::{space0_around, space0_between, space1_before, space1_between, space1_tags_no_case};

impl<'de, E: ParseError<&'de str>> Parse<&'de str, E> for CqlTable<&'de str, CqlIdentifier<&'de str>, CqlIdentifier<&'de str>> {
    fn parse(input: &'de str) -> IResult<&'de str, Self, E> {
        let (input, _) = space1_tags_no_case(["CREATE", "TABLE"])(input)?;
        let (input, if_not_exists) = opt(space1_before(space1_tags_no_case(["IF", "NOT", "EXISTS"])))(input)?;
