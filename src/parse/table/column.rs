...
impl<'de, E: ParseError<&'de str>> Parse<&'de str, E> for CqlColumn<&'de str, CqlIdentifier<&'de str>> {
    fn parse(input: &'de str) -> IResult<&'de str, Self, E> {
        let (input, (name, cql_type)) = space0_between((
            CqlIdentifier::parse,
            CqlType::parse,
        ))(input)?;
        let (input, is_static) = opt(space1_before(tag_no_case("STATIC")))(input)?;
        let (input, is_primary_key) = opt(space1_before(space1_tags_no_case(["PRIMARY", "KEY"])))(input)?;

        Ok((input, CqlColumn::new(
            name,
            cql_type,
            is_static.is_some(),
