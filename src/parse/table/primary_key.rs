...
impl<'de, E: ParseError<&'de str>> Parse<&'de str, E> for CqlPrimaryKey<&'de str> {
    fn parse(input: &'de str) -> IResult<&'de str, Self, E> {
        ...
        Ok((input, CqlPrimaryKey::new(
            partition_key,
            clustering_columns.map(|(_, clustering_columns)| clustering_columns).unwrap_or_default(),
        )))
    }
}